...
def get_char_ngrams(ngram_len, word): 
    ...
    while (begin_idx + ngram_len) <= len(chars): 
        ...
        begin_idx += 1 
    return OrderedDict.fromkeys(ngrams)
    ...
    for url in word_x:
        ...
        for word in words:
            ...
            if (len(ngrams) > max_len_subwords) or \
                ...
                tmp = list(ngrams)[:max_len_subwords]
                all_ngrams.update(OrderedDict.fromkeys(tmp))
                url_in_ngrams.append(tmp) 
                #all_words.add("<UNKNOWN>")
                all_words["<UNKNOWN>"] = None
